...
var detailexporttoExcelButton = document.createElement('input');
detailexporttoExcelButton.setAttribute('id', 'DetailExportToExcel');
detailexporttoExcelButton.setAttribute('value', 'Export to Excel');
detailexporttoExcelButton.setAttribute('type','button');
detailexporttoExcelButton.style.height = '30px';
detailexporttoExcelButton.style.font = 'bold 12px sans-serif';
detailexporttoExcelButton.style.margin= '15px';
detailexporttoExcelButton.style.borderRadius= '25px';
detailexporttoExcelButton.style.display = 'none';

controlAddIn.appendChild(detailexporttoExcelButton);
...
//    return data;
//  }
$(document).on('click', '#DetailExportToExcel', function () {
    if (GloabalFilter.length>2000)
    {
        alert("Information! Number of rows should be less than or equal to 2000.");
    }
    else{
    var innerHtm = $("#tblDrillDown caption").html();
    var attrs = $("#tblDrillDown caption").attrib();
    $("#tblDrillDown caption").remove();
    ...
    $(crit).insertBefore($("#tblDrillDown thead"));
    $("#tblDrillDown").dataTable().fnDestroy();
    var pvt =  crit;

    if (GloabalFilter!=null){
       // JSON.stringify(GloabalFilter);
        console.log(GloabalFilter);
    }
   // alert(JSON.stringify(tableToJSON($("#tblDrillDown"))));
    console.log('<table>' + pvt + '</table>');
    $.ajax({
        url: 'https://DynamicsExport.friedmanllp.com/Home/ExportExcelDetail',
       // timeout:50000,
        // headers: {
        //     'Access-Control-Allow-Origin': '*',
        //     "Access-Control-Allow-Headers": "Origin, X-Requested-With, Content-Type, Accept" 
        // },
        data: { table: '<table>' + pvt + '</table>',JsonData:  JSON.stringify({ GloabalFilter })},
        crossDomain: true,
        type: "POST",
        method: "POST",
                dataType: 'json',
...
console.log(fail);
window.open('https://DynamicsExport.friedmanllp.com' + fail.responseText, '_blank');
        }
    });
    setTimeout(() => {
        $("#tblDrillDown caption").remove();
        ...
        inhtm += '>' + innerHtm + '</caption>';
        $(inhtm).insertBefore($("#tblDrillDown thead"));
        $('#tblDrillDown caption').css('text-align', 'left');
    }, 500);
...
$(document).on('click', '.tblLinkBS', function () {
    ...
    str += '</table>';
    GloabalFilter=filter;
    $('#pivot').hide();
    $('#back').show();
    $('#DetailExportToExcel').show();
    $('#divDetail').show();
    $('#divDetail').html(str);
    $('#tblDrillDown').DataTable({
        "pageLength": 50
